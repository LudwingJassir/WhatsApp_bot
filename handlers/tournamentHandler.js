...
async function startTournament(message) {
    ...
    if (teams.length !== 3)
        return humanReply(message, `⚠️ El torneo necesita exactamente *3 equipos*~\nActualmente hay ${teams.length}. Usa *!team <nombre>* para registrarlos.`);

    const tournament = storage.load('tournament');
    if (tournament.started)
        return humanReply(message, '⚠️ El torneo ya está en curso~ Usa *!standings* para ver el estado.');
    ...
    storage.save('tournament', {
        ...
        tiebreaker:       false,
        tiebreakerRounds: [],
        tiebreakerPhase:  0   // 0=no iniciado, 1=TB R1 jugado, 2=finalizado
    });

    return humanReply(message,
        `🏆 *¡TORNEO MLBB INICIADO, GANBARIMASU~!* 🏆\n\n` +
        `⚔️ *Ronda 1*\n🛡️ ${A} *vs* ${B} 🛡️\n\n` +
        `⚔️ *Ronda 2*\n🛡️ ${B} *vs* ${C} 🛡️\n\n` +
        `⚔️ *Ronda 3*\n🛡️ ${A} *vs* ${C} 🛡️\n\n` +
...
async function registerWin(message, args) {
    ...
    if (!tournament.started)  return humanReply(message, '⚠️ El torneo no ha iniciado~ Usa *!startTournament*');
    // Si el torneo terminó y hay finalistas pero sin ganador final → registrar ganador de la final
    if (tournament.finished && tournament.finalists.length >= 2 && !tournament.finalWinner)
        return registerFinalWin(message, args);
    if (tournament.finished)  return humanReply(message, '🏆 ¡El torneo ya finalizó! Usa *!final* para ver el campeon~');
    if (!teams[winner])       return humanReply(message, `❌ El equipo *${winner}* no existe~`);
    ...
    const loser = pendingRound.teamA === winner ? pendingRound.teamB : pendingRound.teamA;
...
async function evaluateStandings(message, tournament, teams) {
    ...
    return generateFinal(message, tournament, teams, [first.name, second.name], sorted);
}
...
async function initTiebreaker(message, tournament, teams, sorted) {
    ...
    const tiebreakerRounds = [
        { round: 'TB1', teamA: tb1A, teamB: tb1B,      winner: null, description: `Los 2 equipos con más kills` },
        { round: 'TB2', teamA: null, teamB: tbOutsider, winner: null, description: `Perdedor TB1 vs ${tbOutsider}` }
    ];
    ...
    const killRanking = byKills.map((t, i) =>
        `${i + 1}. 🗡️ *${t.name}* — ${t.kills} kills`
    ).join('\n');
    ...
    return humanReply(message,
        ...
        `━━━━━━━━━━━━━━━━━━\n\n` +
        `💡 *${tb1A}* y *${tb1B}* tienen segunda oportunidad por su desempeño en kills~\n\n` +
...
async function showStandings(message) {
    ...
    if (names.length === 0) return humanReply(message, '📭 No hay equipos registrados~');
    ...
    return humanReply(message, `📊 *TABLA DE POSICIONES*\n${status}${tbInfo}\n\n${rows}`);
}

// ─── Registrar estadísticas ───────────────────────────────────────────────────
async function registerStats(message, args) {
    if (args.length < 2) return humanReply(message, '⚠️ Uso: *!stats <equipo> <número>kills*\nEjemplo: !stats Alpha 25kills');
    ...
    if (isNaN(kills))     return humanReply(message, '⚠️ Formato incorrecto~ Ejemplo: *!stats Alpha 25kills*');
    teams[teamName].kills += kills;
    storage.save('teams', teams);
    return humanReply(message, `📈 *Stats actualizados para ${teamName}~*\n🗡️ Total kills: *${teams[teamName].kills}*`);
}
...
async function showFinal(message) {
    ...
    if (!tournament.finished || tournament.finalists.length < 2)
        return humanReply(message, '⏳ La final aún no está disponible~ Completa todas las rondas primero.');
...
async function registerFinalWin(message, args) {
    ...
    if (tournament.finalWinner)
        return humanReply(message, `🏆 La final ya fue registrada~ Ganó *${tournament.finalWinner}*`);
    ...
    await humanReply(message, `✅ *¡Ganador de la Final registrado!*\n\n🏆 *${winner}* ¡es el CAMPEÓN! 👑`);
...
async function recalculateTiebreaker(message) {
    ...
    // Validaciones
    if (!tournament.started)
        return humanReply(message, '⚠️ El torneo no ha iniciado~ Usa *!startTournament*');

    if (tournament.finished)
        return humanReply(message, '🏆 El torneo ya finalizó~ No se puede recalcular el tiebreaker.');
    ...
    if (!(first.points === 1 && second.points === 1 && third.points === 1))
        return humanReply(message,
            ...
            `📊 Tabla actual:\n` +
            sorted.map((t, i) => `${i+1}. *${t.name}* — ${t.points} pts | ${t.kills} kills`).join('\n') + '\n\n' +
...
module.exports = { startTournament, registerWin, showStandings, registerStats, showFinal, recalculateTiebreaker, registerFinalWin };